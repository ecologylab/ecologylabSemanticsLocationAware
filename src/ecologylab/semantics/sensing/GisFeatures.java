...
/**
 * Tools for extracting GIS data from EXIF+ headrs from image files, and using these to populate
 * appropriate S.IM.PL Metadata.
 * 
 * @author andruid
 */
public class GisFeatures extends Debug
{
	...
	public static final MetadataExifFeature	GPS_IMG_DIRECTION_REF_FEATURE	= new MetadataExifFeature(
																																						"direction_ref",
																																						GpsDirectory.TAG_GPS_IMG_DIRECTION_REF);

	public static final MetadataExifFeature	GPS_IMG_DIRECTION_FEATURE			= new MetadataExifFeature(
																																						"direction",
																																						GpsDirectory.TAG_GPS_IMG_DIRECTION);
	...
	public static final MetadataExifFeature	GPS_METADATA_FEATURES[]				=
			...
			GPS_ALTITUDE_FEATURE, GPS_IMG_DIRECTION_FEATURE, GPS_IMG_DIRECTION_REF_FEATURE,
	...
	/**
	 * Construct one of these, and use it to get nice dms GPS data from the exif metadata -- if the
	 * GPS stuff is there.
	 * 
	 * @param gpsDir
	...
	{
		GisLocation result = containsGisMixin(parentMetadata) ? parentMetadata
				.getMixin(GisLocation.class) : new GisLocation();
	...
	public static Metadata extractMixin(com.drew.metadata.Directory gpsDir,
			final SemanticsGlobalScope semanticsScope, final Metadata parentMetadata)
	{
		...
		ParsedURL reverseGeo = ParsedURL.getAbsolute(YAHOO_REVERSE_GEO + result.getLongitude() + ","
				+ result.getLatitude());
		...
		geoClosure.addContinuation(new Continuation<DocumentClosure>()
		{
			...
			public void callback(DocumentClosure o)
			{
				...
				if (yahooResult instanceof YahooGeoCodeResult)
				{
					...
					PostalAddress postalAddress = new PostalAddress(semanticsScope
							.getMetaMetadataRepository().getMMByClass(PostalAddress.class));
